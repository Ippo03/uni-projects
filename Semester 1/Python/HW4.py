...
def neighbors(p):
    ...
    """GRAPSTE TON KWDIKA SAS EDW."""
    i, j = p
    set = {(i,j+1),(i,j-1),(i-1,j-1),(i-1,j+1),(i+1,j+1),(i-1,j),(i+1,j),(i+1,j-1)}
    return set
...
def tick(board):
    ...
    make_false = []
    make_true = []
    for every_cell, alive in get_iterator(board):
        alive_neighbor_count = 0
        neighbor = list(neighbors(every_cell))
        for k in range(len(neighbor)):
            i = neighbor[k][0]
            j = neighbor[k][1]
            if(is_alive(board,neighbor[k]) and (i >= 0 and j >= 0)):
                alive_neighbor_count += 1
        if (alive_neighbor_count <= 1 or alive_neighbor_count >= 4) and alive:
            make_false.append(every_cell)
        elif alive_neighbor_count == 3 and not alive:
            make_true.append(every_cell)
    for cell in make_false:
        set_alive(board,cell,False)
    for cell in make_true:
