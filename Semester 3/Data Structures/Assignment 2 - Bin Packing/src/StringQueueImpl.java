...
public class StringQueueImpl<T> implements StringQueue<T> {          //class Queue implements methods of interface stack(we use generics)
    ...
    @Override
    public void printQueue(PrintStream stream) {                    //method tha prints the elements of the queue from the first to the last node of the queue
        ...
        if(tail != null){
            stream.print(tail.value); 
        }                                                          //prints the value of the last node of the queue 
